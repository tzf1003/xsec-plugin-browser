...
class BrowserController {
  constructor(host) {
    ...
    this.onVisibilityChange = () => void this.handleVisibilityChange(); this.onWindowBlur = () => { this.releasePressedKeys(); this.releaseMouseButtons(); };
  }
  mount(root, context) {
  ...
  clearSurfaceInput() { this.clearPendingMove(); this.mouseSurfaceId = ""; this.compositionCancelled ||= this.composing; this.composing = false; this.compositionSurfaceId = ""; if (this.controls?.keyboard) this.controls.keyboard.value = ""; this.pressedKeys.clear(); this.inputTail = Promise.resolve(); } releasePressedKeys() { const held = [...this.pressedKeys.values()]; this.pressedKeys.clear(); held.forEach((input) => void this.sendInput({ ...input, event_type: "up", auto_repeat: false })); }
  ...
  async closeSurface(clearState = true, invalidate = true) {
    const surface = this.surface; const subscription = this.subscription; const heldKeys = [...this.pressedKeys.values()]; const release = this.releaseMouseButtons(); if (invalidate) this.surfaceGeneration += 1; this.frameSurfaceId = ""; this.controls?.canvas && (this.controls.canvas.hidden = true); const keyRelease = surface ? this.inputTail.then(async () => { for (const input of heldKeys) await this.host.request("xsec.browser.surface.input", { surfaceId: surface.id, input: { ...input, event_type: "up", auto_repeat: false } }); }) : Promise.resolve(); const results = await Promise.allSettled([keyRelease, release]); this.surface = undefined; this.subscription = undefined; this.clearSurfaceInput(); subscription?.dispose?.(); const failure = results.find((result) => result.status === "rejected"); if (failure) throw failure.reason;
  ...
  releaseMouseButtons() { const buttons = this.mouseButtons; const surfaceId = this.mouseSurfaceId || this.surface?.id; const move = this.takePendingMove(); this.mouseButtons = 0; this.mouseSurfaceId = ""; if (!buttons || !surfaceId) return Promise.resolve(); const releases = MOUSE_BUTTON_MASKS.flatMap((mask, index) => buttons & mask ? [{ kind: "mouse", event_type: "up", ...this.lastPoint, button: mouseButton(index), buttons: NO_MOUSE_BUTTONS, modifiers: 0 }] : []); return this.queuePointer(surfaceId, [...(move ? [move] : []), ...releases]); }
  sendInput(input, surfaceId = this.surface?.id) {
    if (!surfaceId) return Promise.resolve(); const send = async () => { if (this.surface?.id !== surfaceId) return; try { await this.host.request("xsec.browser.surface.input", { surfaceId, input }); } catch (error) { void this.failSurface(error, surfaceId); } };
  ...
  queuePointer(surfaceId, inputs) { let tail = Promise.resolve(); inputs.forEach((input) => { tail = this.sendInput(input, surfaceId); }); return tail; }
...
function workspaceControls(controller) {
  ...
  const address = element("input"); address.placeholder = "输入授权范围内的 HTTP/HTTPS 地址"; address.setAttribute("aria-label", "浏览器地址"); address.oninput = () => { controller.state.address = address.value; }; address.onblur = () => controller.updateAddress(true); address.onkeydown = (event) => { if (event.key === "Enter" && !controller.state.busy) void controller.navigate(); }; nav.append(back, forward, reload, stop, address);
...
function bindSurfaceInput(controller, canvas, keyboard) {
  ...
  canvas.onwheel = (event) => { event.preventDefault(); const move = controller.takePendingMove(); const point = controller.point(event); void controller.queuePointer(controller.surface?.id, [...(move ? [move] : []), { kind: "mouse", event_type: "wheel", ...point, button: "none", modifiers: eventModifiers(event), delta_x: event.deltaX, delta_y: event.deltaY }]); };
