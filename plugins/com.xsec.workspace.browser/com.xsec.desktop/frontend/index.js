...
class BrowserController {
  ...
  render() {
    ...
    const tabKey = JSON.stringify([this.state.pageId, this.state.busy, this.state.pages.map((item) => [item.id, item.title, item.url]), this.state.groups.map((item) => [item.current_page_id, item.owner])]); if (tabKey !== this.tabControlsKey) { const active = document.activeElement; const pageId = active?.closest?.(".tab")?.dataset.pageId; const className = active?.className; this.controls.tabs.replaceChildren(...this.state.pages.map((item) => pageTab(this, item)), this.controls.newTab); if (pageId && (className === "tab-open" || className === "close")) this.controls.tabs.querySelector(`.tab[data-page-id="${CSS.escape(pageId)}"] .${className}`)?.focus({ preventScroll: true }); this.tabControlsKey = tabKey; }
