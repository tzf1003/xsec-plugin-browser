...
class BrowserController {
  ...
  clearSurfaceInput() { this.clearPendingMove(); this.mouseSurfaceId = ""; this.closingSurfaceId = ""; this.compositionCancelled ||= this.composing; this.composing = false; this.compositionSurfaceId = ""; if (this.controls?.keyboard) this.controls.keyboard.value = ""; this.pressedKeys.clear(); this.inputTail = Promise.resolve(); } releasePressedKeys() { const held = [...this.pressedKeys.values()]; this.pressedKeys.clear(); held.forEach((input) => void this.sendInput({ ...input, event_type: "up", auto_repeat: false })); }
  ...
  async closeSurface(clearState = true, invalidate = true) { if (this.surfaceTeardownPromise) return this.surfaceTeardownPromise; this.surfaceTeardownPromise = this.closeSurfaceNow(clearState, invalidate).finally(() => { this.surfaceTeardownPromise = undefined; }); return this.surfaceTeardownPromise;
    ...
    const surface = this.surface; const subscription = this.subscription; const heldKeys = [...this.pressedKeys.values()]; this.closingSurfaceId = this.mouseSurfaceId || surface?.id || ""; const release = this.releaseMouseButtons(); this.surface = undefined; this.subscription = undefined; if (invalidate) this.surfaceGeneration += 1; this.frameSurfaceId = ""; this.controls?.canvas && (this.controls.canvas.hidden = true); const keyRelease = surface ? this.inputTail.then(async () => { for (const input of heldKeys) await this.host.request("xsec.browser.surface.input", { surfaceId: surface.id, input: { ...input, event_type: "up", auto_repeat: false } }); }) : Promise.resolve(); const results = await Promise.allSettled([keyRelease, release]); this.clearSurfaceInput(); subscription?.dispose?.(); if (clearState) this.state.surfaceState = ""; await this.closeNativeSurface(surface); const failure = results.find((result) => result.status === "rejected"); if (failure) throw failure.reason;
  ...
  releaseMouseButtons() { const buttons = this.mouseButtons; const surfaceId = this.mouseSurfaceId || this.surface?.id; const move = this.takePendingMove(); this.mouseButtons = 0; this.mouseSurfaceId = ""; if (!buttons || !surfaceId) return Promise.resolve(); return this.queuePointer(surfaceId, [...(move ? [move] : []), ...mouseButtonReleases(buttons, this.lastPoint, 0)]); }
  sendInput(input, surfaceId = this.surface?.id) {
    if (!surfaceId || this.surface?.id !== surfaceId && this.closingSurfaceId !== surfaceId) return Promise.resolve(); const send = async () => { if (this.surface?.id !== surfaceId && this.closingSurfaceId !== surfaceId) return; try { await this.host.request("xsec.browser.surface.input", { surfaceId, input }); } catch (error) { void this.failSurface(error, surfaceId); } };
