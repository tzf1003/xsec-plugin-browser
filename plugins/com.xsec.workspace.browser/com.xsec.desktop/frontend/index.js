...
class BrowserController {
  ...
  async closeSurface(clearState = true, invalidate = true) {
    const surface = this.surface; const subscription = this.subscription; const pendingInput = this.inputTail; const release = this.releaseMouseButtons(); if (invalidate) this.surfaceGeneration += 1; this.frameSurfaceId = ""; this.controls?.canvas && (this.controls.canvas.hidden = true); await Promise.all([pendingInput, release]); this.surface = undefined; this.subscription = undefined; this.clearSurfaceInput(); subscription?.dispose?.();
  ...
  sendInput(input, surfaceId = this.surface?.id) {
    if (!surfaceId) return Promise.resolve(); const send = async () => { if (this.surface?.id !== surfaceId) return; try { await this.host.request("xsec.browser.surface.input", { surfaceId, input }); } catch (error) { await this.failSurface(error, surfaceId); } };
