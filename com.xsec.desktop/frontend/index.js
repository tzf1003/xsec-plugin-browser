...
const NEW_PAGE_GRACE_MS = 5000, POINTER_MOVE_MS = 33, FRAME_ACK_INTERVAL_MS = 40, VIEWPORT_WIDTH = 1280, VIEWPORT_HEIGHT = 800, MAX_FRAME_WIDTH = VIEWPORT_WIDTH * 4, MAX_FRAME_HEIGHT = VIEWPORT_HEIGHT * 4, MAX_FRAME_JPEG_BYTES = 8 * 1024 * 1024, NO_MOUSE_BUTTONS = 0, MOUSE_BUTTON_MASKS = [1, 4, 2, 8, 16];
...
const SVG_NAMESPACE = "http://www.w3.org/2000/svg", ICON_SIZE = "16";
const LUCIDE_ICONS = { globe: [["circle", { cx: "12", cy: "12", r: "10" }], ["path", { d: "M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20" }], ["path", { d: "M2 12h20" }]], x: [["path", { d: "M18 6 6 18" }], ["path", { d: "m6 6 12 12" }]], plus: [["path", { d: "M5 12h14" }], ["path", { d: "M12 5v14" }]], radio: [["path", { d: "M4.9 19.1C1 15.2 1 8.8 4.9 4.9" }], ["path", { d: "M7.8 16.2c-2.3-2.3-2.3-6.1 0-8.5" }], ["circle", { cx: "12", cy: "12", r: "2" }], ["path", { d: "M16.2 7.8c2.3 2.3 2.3 6.1 0 8.5" }], ["path", { d: "M19.1 4.9C23 8.8 23 15.1 19.1 19" }]], maximize2: [["polyline", { points: "15 3 21 3 21 9" }], ["polyline", { points: "9 21 3 21 3 15" }], ["line", { x1: "21", x2: "14", y1: "3", y2: "10" }], ["line", { x1: "3", x2: "10", y1: "21", y2: "14" }]], minimize2: [["polyline", { points: "4 14 10 14 10 20" }], ["polyline", { points: "20 10 14 10 14 4" }], ["line", { x1: "14", x2: "21", y1: "10", y2: "3" }], ["line", { x1: "3", x2: "10", y1: "21", y2: "14" }]], refreshCw: [["path", { d: "M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8" }], ["path", { d: "M21 3v5h-5" }], ["path", { d: "M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16" }], ["path", { d: "M8 16H3v5" }]], arrowLeft: [["path", { d: "m12 19-7-7 7-7" }], ["path", { d: "M19 12H5" }]], arrowRight: [["path", { d: "M5 12h14" }], ["path", { d: "m12 5 7 7-7 7" }]], square: [["rect", { width: "18", height: "18", x: "3", y: "3", rx: "2" }]] };
function lucideIcon(name) { const svg = document.createElementNS(SVG_NAMESPACE, "svg"); const attributes = { "aria-hidden": "true", fill: "none", height: ICON_SIZE, stroke: "currentColor", "stroke-linecap": "round", "stroke-linejoin": "round", "stroke-width": "2", viewBox: "0 0 24 24", width: ICON_SIZE }; Object.entries(attributes).forEach(([key, value]) => svg.setAttribute(key, value)); for (const [tag, values] of LUCIDE_ICONS[name]) { const shape = document.createElementNS(SVG_NAMESPACE, tag); Object.entries(values).forEach(([key, value]) => shape.setAttribute(key, value)); svg.append(shape); } return svg; }
function setIcon(button, name) { if (button.dataset.icon === name) return; button.dataset.icon = name; button.replaceChildren(lucideIcon(name)); }
...
function validateChromePath(value) { const path = value.trim(); if (!path) return ""; if (/[\0-\x1F\x7F]/.test(path)) throw new Error("浏览器路径包含非法字符"); if (path.startsWith("\\\\?\\")) { const extendedPath = path.slice(4); if (/^UNC\\/i.test(extendedPath) || !/^[A-Za-z]:[\\/]/.test(extendedPath)) throw new Error("请输入浏览器可执行文件的绝对路径"); return path; } if (path.startsWith("\\\\")) throw new Error("不支持网络共享路径"); if (path.startsWith("/") || /^[A-Za-z]:[\\/]/.test(path)) return path; throw new Error("请输入浏览器可执行文件的绝对路径"); }
...
function selectSession(sessions, currentId, runId, userSelected) { const current = sessions.find((session) => session.id === currentId); if (current && userSelected) return current.id; const matched = runId ? sessions.find((session) => session.live && session.run_id === runId) : undefined; return matched?.id || current?.id || sessions.find((session) => session.live)?.id || sessions[0]?.id || null; }
...
class BrowserController {
  ...
  async ensureSurface() { const run = () => this.ensureSurfaceNow(); this.surfaceTransitionTail = this.surfaceTransitionTail.then(run, run); return this.surfaceTransitionTail; } async ensureSurfaceNow() {
    ...
    try {
      ...
      if (!this.surfaceRequestCurrent(generation, session.id, page.id) || this.surface?.id !== result.surfaceId) {
        ...
        if (stale) { this.surface = undefined; this.subscription?.dispose?.(); this.subscription = undefined; this.frameViewport = undefined; this.frameSurfaceId = ""; this.clearSurfaceInput(); }
  ...
  async presentFrame(frame, surfaceId) {
    try {
      ...
      if (image.width > MAX_FRAME_WIDTH || image.height > MAX_FRAME_HEIGHT) { image.close(); throw new Error("浏览器画面 JPEG 尺寸超出限制"); }
      const firstFrame = this.frameSurfaceId !== surfaceId; const canvas = this.controls.canvas; if (canvas.width !== image.width) canvas.width = image.width; if (canvas.height !== image.height) canvas.height = image.height; this.canvasContext ||= canvas.getContext("2d", { alpha: false }); this.canvasContext?.drawImage(image, 0, 0); image.close(); this.frameViewport = { width: frame.width, height: frame.height }; this.frameSurfaceId = surfaceId;
  ...
  async failSurface(error, surfaceId, generation) {
    ...
    if (current) { this.state.surfaceState = "error"; this.state.surfaceError = errorText(error); this.frameViewport = undefined; this.frameSurfaceId = ""; this.render(); } if (surface) await this.closeSurface(false);
  }
  ...
  point(event) {
    const rect = this.controls.canvas.getBoundingClientRect(); const width = this.frameViewport?.width || VIEWPORT_WIDTH; const height = this.frameViewport?.height || VIEWPORT_HEIGHT;
  ...
  render() {
    ...
    const tabKey = JSON.stringify([this.state.pageId, this.state.busy, this.state.pages.map((item) => [item.id, item.title, item.url, this.closingPages.has(item.id)]), this.state.groups.map((item) => [item.current_page_id, item.owner])]); if (tabKey !== this.tabControlsKey) { const active = document.activeElement; const pageId = active?.closest?.(".tab")?.dataset.pageId; const className = active?.className; this.controls.tabs.replaceChildren(...this.state.pages.map((item) => pageTab(this, item))); if (pageId && (className === "tab-open" || className === "close")) { const restored = this.controls.tabs.querySelector(`.tab[data-page-id="${CSS.escape(pageId)}"] .${className}`); const tab = restored?.closest(".tab"); const target = restored && !restored.disabled ? restored : tab?.querySelector(".tab-open:not(:disabled)") || tab?.nextElementSibling?.querySelector(".tab-open:not(:disabled)") || tab?.previousElementSibling?.querySelector(".tab-open:not(:disabled)") || this.controls.tabs.querySelector(".tab-open:not(:disabled)") || (live && !this.state.busy ? this.controls.newTab : null); target?.focus({ preventScroll: true }); } this.tabControlsKey = tabKey; }
    ...
    this.controls.follow.classList.toggle("active", this.state.follow); this.controls.focus.classList.toggle("active", this.state.focus); this.controls.follow.setAttribute("aria-pressed", String(this.state.follow)); this.controls.focus.setAttribute("aria-pressed", String(this.state.focus)); setIcon(this.controls.focus, this.state.focus ? "minimize2" : "maximize2"); this.controls.focus.setAttribute("aria-label", this.state.focus ? "退出浏览器专注模式" : "进入浏览器专注模式"); this.controls.focus.title = this.controls.focus.getAttribute("aria-label"); this.controls.error.textContent = this.state.error; this.controls.error.hidden = !this.state.error; this.controls.address.disabled = Boolean(this.state.busy); this.controls.canvas.hidden = !live || !page || this.frameSurfaceId !== this.surface?.id;
    ...
    this.controls.newTab.disabled = !live || Boolean(this.state.busy); const newTabLabel = !live ? "浏览器会话未运行，无法新建标签页" : this.state.busy ? "浏览器正在处理操作" : "新建浏览器标签页"; this.controls.newTab.setAttribute("aria-label", newTabLabel); this.controls.newTab.title = newTabLabel; this.controls.actions.forEach((button) => { button.disabled = !live || !page || Boolean(this.state.busy); });
  ...
  buildWorkspace() {
    this.root.replaceChildren(element("style", "", workspaceCss)); this.controls = workspaceControls(this); this.root.append(this.controls.app); window.addEventListener("keydown", this.onEscape); window.addEventListener("blur", this.onWindowBlur); document.addEventListener("visibilitychange", this.onVisibilityChange); this.render();
  }
  buildSettings() {
...
function sessionOption(session, selected) { if (!session) { const option = element("option", "", "未匹配当前浏览器会话"); option.value = ""; option.disabled = true; option.selected = true; return option; } const option = element("option", "", `${session.live ? "运行中" : "已结束"} · ${session.id.slice(0, 10)}`); option.value = session.id; option.selected = selected; return option; }
function pageTab(controller, page) {
  ...
  const closing = controller.closingPages.has(page.id); tab.dataset.pageId = page.id; open.type = "button"; open.disabled = closing; if (page.id === controller.state.pageId) open.setAttribute("aria-current", "page"); open.append(lucideIcon("globe"), element("span", "label", pageLabel(page)));
  ...
  open.onclick = () => void controller.choosePage(page.id).catch((error) => controller.fail(error, "page-select")); close.type = "button"; close.setAttribute("aria-label", `关闭 ${pageLabel(page)}`); close.title = close.getAttribute("aria-label"); close.append(lucideIcon("x")); close.disabled = !controller.session()?.live || Boolean(controller.state.busy) || closing; close.onclick = () => void controller.closePage(page.id); tab.append(open, close); return tab;
}
function icon(name, label, handler) {
  const button = element("button", "icon"); button.type = "button"; button.setAttribute("aria-label", label); button.title = label; button.append(lucideIcon(name)); button.dataset.icon = name; button.onclick = handler; return button;
}
function workspaceControls(controller) {
  ...
  const nav = element("div", "nav"); const back = icon("arrowLeft", "浏览器后退", () => void controller.pageAction("back")); const forward = icon("arrowRight", "浏览器前进", () => void controller.pageAction("forward")); const reload = icon("refreshCw", "浏览器刷新", () => void controller.pageAction("reload")); const stop = icon("square", "停止加载", () => void controller.pageAction("stop"));
  ...
  const toolbar = element("div", "toolbar"); toolbar.setAttribute("role", "toolbar"); toolbar.setAttribute("aria-label", "浏览器功能区"); toolbar.append(session, nav); const footer = element("footer", "footer"); app.append(tabbar, toolbar, error, stage, footer); bindSurfaceInput(controller, canvas, keyboard);
